...
contract("ThePromisedMoon", function (accounts) {
    ...
    */
    ...
    it('Add liq', async function(){
        try
        {
            await ctrTK.addLiqContract({ value: toWei(200), from: main_account });
    ...
    it('Perform sell should work', async function(){
        ...
        {
            // Perform sell
            let tokensSell = await ctrTK.balanceOf(accounts[1]);
            log(`Can adr ${accounts[1]} sell? ${(await ctrTK.canAdrSellCD(accounts[1])).toString()}, liq pair balance: ${(await ctrTK.balanceOf(pairAdr))}`);
            // Token from pair balance? TODO
            await sellDEX(tokensSell.div(BN2(10)).toString(), accounts[1]);
            await checkMcapVSDatafeed();
            return assert.isTrue(true);
        } catch (err) {
            console.log(err.toString());
